...
export const getStyles = (props: StylesProps) => {
  const { theme, radius, size, isInvalid } = props;
  const colourStyles = {
    option: (styles: any) => {
      return {
        ...styles,
    ...
    control: (base: any, state: any) => ({
      ...
      '&:hover': {
        boxShadow: isInvalid ? `0 0 0 0.9x ${theme.colors.red[5]}` : ``,
      },
      boxShadow: isInvalid
        ? `0 0 0 0.4px ${theme.colors.red[5]}`
...
export const useStyles = (props: ReactSelectStylesProps) => {
  const { themeOverride, radius, size, isInvalid } = props;
  const theme: BeautifyTheme = useBeautifyTheme(themeOverride);

  return useMemo(
    () => getStyles({ theme, isInvalid, radius, size }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [themeOverride, radius, size, isInvalid]
  );
};
